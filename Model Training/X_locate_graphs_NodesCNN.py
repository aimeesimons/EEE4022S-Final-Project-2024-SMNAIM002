...
class NodeGCN(nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        #forward pass of nerual network
        x = self.conv1d_1(x)
        x = self.relu(x)
        x = self.conv1d_2(x)
        x = self.relu(x)
...
def train(model, data, optimizer, criterion):
    model.train() # put model in evalutation mode
    total_loss = 0
    for batch in data:
        optimizer.zero_grad()  # Zero the gradients
        out = model(batch.x.float(), batch.edge_index, batch.batch)  # Forward pass
...
if __name__ =='__main__':
    with open("GraphList_Nodes_classify1.pickle","rb") as f:
        graph_list_edges = pickle.load(f)
    print("Loaded Data")
    
    graph_list_edges_train, graph_list_edges_test = train_test_split(graph_list_edges, test_size=0.2, random_state=42, shuffle=True) #split data in training and testing
    ...
    modelNode = NodeGCN(in_channels, hidden_channels, out_channels, fc_hidden_channels)
    optimizer = torch.optim.Adam(modelNode.parameters(), lr=learning_rate)
    ...
    for epoch in range(num_epochs):
        loss = train(modelNode, train_loader, optimizer, criterion)
        losses.append(loss)
    ...
    acc, predictions, actual = evaluate(modelNode, test_loader, predictions, actual)
    print(f'Test Accuracy: {acc:.4f}')

    torch.save(modelNode, 'model_node_2.pth')
    ...
    with open("graph_nodes_ordinal_encoder1.pkl", 'rb') as f:
    ...
    #Display Classificiation Metrics
